...
export function SurveyCard() {
    ...
    if (step === 8) {
      ...
      try {
        ...
        const payload = {
          ...
          source: 'Pure Growth Investments LLC - Survey',
        ...
        if (typeof window !== 'undefined' && (window as { fbq?: (...args: unknown[]) => void }).fbq) {
          ...
          if (qualified) {
            fbq('track', 'Lead', {
              ...
              content_name: 'Pure Growth Investments Survey', content_category: 'real_estate',
  ...
  const handleAddressSelect = (address: string, details: AddressDetails) => {
    ...
    // Auto-advance to next step after address selection (only if in Massachusetts area)
    setTimeout(() => {
      setStep(2)
    }, 300)
  }
    ...
    return (
      <div className="w-full max-w-2xl rounded-2xl border border-gray-200 bg-white p-6 shadow-lg">
        <div className="flex flex-col items-center gap-5 text-center">
          ...
          >
            Call Us: Pure Growth Investments LLC
          </a>
        </div>
      </div>
    )
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-center pt-2">
              We currently only buy houses in the <strong>DC, Maryland, and Virginia (Massachusetts)</strong> area and surrounding suburbs.
            </DialogDescription>
          </DialogHeader>
